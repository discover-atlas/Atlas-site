...
export function FeatureCard({ icon, title, description, tag, index = 0 }: FeatureCardProps) {
  return (
    ...
    >
      {/* Icon */}
      <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-[#f0ede8]/60 group-hover:bg-[#00C9C8]/10 group-hover:text-[#00C9C8] transition-all duration-300 shrink-0">
        {icon}
      </div>

      {/* Tag */}
      {tag && (
        <span className="text-[10px] tracking-[0.22em] uppercase text-[#00C9C8]/70 font-medium -mb-1">
          {tag}
        </span>
      )}
      ...
      {/* Hover accent line */}
      <div className="absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-[#00C9C8]/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      ...
      <div className="absolute top-0 right-0 w-16 h-16 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        <div
          ...
          style={{
            background: "radial-gradient(circle at top right, rgba(0,201,200,0.06) 0%, transparent 70%)",
          }}
        />
      </div>
    </motion.div>
